...
public int degree() {
	int deg  = coeff.length - 1; // deg = length-1 because there's always one less degree than coefficients because of the 0 degree
return deg;
		}
		...
		public int evaluate (int x) //method to evaluate a polynomial given x
		{
			...
			int co = 0;//since coefficients go from highest degree to lowest, start at 0 because thats the highest degree
				for (int j=deg;j>=0;j--) // since coefficient is stating at 0, which is the coefficient for highest degree, start with the highest degree
					{
		...
		if(a.deg == b.deg) { //if they are the same degree simply add them
			...
			for(int i=0; i<b.coeff.length;i++) {
				a.coeff[i+x] = a.coeff[i+x] + b.coeff[i]; //once you know how much bigger a is, the coefficients for degrees > b stay the same and add the other coefficients
		}
		}
...
public void Multiply(Polynomial b) {//hardest part by far  start by creating an array one less than the two polynomials lengths added together
	Polynomial a = this;
	Polynomial temp = new Polynomial(a.coeff);
	double [] mult = new double[(a.coeff.length+b.coeff.length)-1];
for (int i=0;i<a.coeff.length;i++)
	for(int j=0;j<b.coeff.length;j++) {
		mult[i+j] += temp.coeff[i] * b.coeff[j];//in the loop, multiply the coefficients together and if there are still coefficients with the same degree add them together
